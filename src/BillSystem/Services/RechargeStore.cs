...
///
/// 学校那边本来就存着完整的历史，这份本地副本是为了离线也能看，以及"这次比上次多了几笔"
/// 这种判断——每次充完值都拉一遍，多出来的就是新的。
///
/// <b>文件里是正序的</b>（最早的在第一行，最新的在最后一行），跟 readings 那个文件一样，
/// 自己翻 jsonl 顺着读下来就是时间顺序；内存里那份反过来排（<see cref="Snapshot"/>），
/// 因为界面要的是"最近一笔在最上面"。
/// </summary>
public sealed class RechargeStore
{
    ...
    private void Load()
    {
        ...
        Sort();
        // 早先的版本是照服务器给的倒序往后追加的，载入时顺手把文件理成正序，
        // 重复行和坏行一并清掉（行数对不上就说明有）
        if (!ordered || fileLines != _records.Count) Save();
    }

    /// <summary>
    /// 把整份记录重写一遍，文件里按支付时间正序。合并进来的可能是比现有记录更早的
    /// （第一次拉全量就是），追加写没法保证顺序，索性每次都整份落一次——总共一百来行，
    /// 一年也就多几十行，重写的开销可以忽略。
    ///
