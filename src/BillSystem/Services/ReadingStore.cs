...
public sealed class ReadingStore
{
    ...
    public bool TryAdd(Reading r)
    {
        lock (_gate)
        {
            ...
            if (_bySlot.TryGetValue(r.SlotTime, out int at))
            {
                ...
                _readings[at] = r;
                // 读到的跟这一格里已经有的一模一样（电表还没上传新读数）：不用再写一行，
                // 文件里那条已经说明这一格是什么了。这也是文件里重复行最主要的来源
    ...
    /// <summary>
    /// 载入完顺手把文件收拢成"一格一行"。文件是只追加的，同一个整点重复查到就多一行
    /// （载入时后写的赢），行数会慢慢多于图表上的格子数——自己翻 jsonl 的时候很容易看糊。
    /// 顺带把载入时跳过的坏行一起清掉。
    ///
    /// 只在真有多余的行时才重写，而且先写临时文件再整体替换，中途出事最多丢这一次收拢。
    /// </summary>
