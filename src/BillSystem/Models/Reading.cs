...
{
    /// <summary>
    /// 这条读数算在哪个整点上（本机时间，一定是 xx:00:00）。一个整点只留一条，两条相邻读数的
    /// 差值就是它们之间那一个小时的用电量——3:00 那条减 2:00 那条，记在<b>两点</b>这一格里。
    /// </summary>
    ...
    ///
    /// 整点轮询偶尔会早几十毫秒，2:59:59.98 查到的那条得算 3:00 的读数；半点那一次（3:30）和
    /// 程序刚启动时那一次（23:42）都该记在<b>本整点</b>——按"最近的整点"算就借用了还没到的下一格，
    /// 等下一个整点真查一次又把它盖掉，本该有数的那格反倒空着。
    /// </summary>
