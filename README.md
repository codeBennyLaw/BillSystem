...
设置分四页，所有改动都是**点保存才生效**，点取消就当没改过。

"宿舍"页管在记录哪几间（默认空的，自己加）：

![设置 · 宿舍](docs/settings-dorms.png)

"提醒"页：两条提醒线和发件邮箱是几间共用的，**弹不弹通知、发不发邮件、发到哪几个邮箱是按间配的**：
...
- **充值在柱子顶上点一个小三角，柱子不变色**：小时和日粒度下，充过电的那一格柱顶上方有个绿色小三角，鼠标移上去气泡里写清"充值  60 元 · 90.0 度"（度数是金额 ÷ 电价换出来的）。柱状图本身只表示剩余电量，不拿颜色去标充值——按度数比例填一截绿的话，月粒度下整根柱子都会变绿（一个月充进来的度数差不多总比月底剩下的多），金色的剩余电量反倒看不见了。三角点在哪一格：**充值之后剩余电量往上跳的那一格**（钱是这个钟付的，但电表两三个小时才抄一次，跳上去往往是后一格的事，所以往后最多找 4 格），**挑涨幅离"该充进来多少度"最近的那一格**——光看"涨了没有"会被半路的小波动骗走（电表修正过读数，或者同一段里还有第二笔充值）；都没涨就点在付款那一格。**月和年粒度不点三角**：那一格里往往好几笔，点了也说不清是哪一笔，金额和度数照样写在气泡里。这一屏里一笔充值都没有时，图例上也不摆"充值"那一条。
- **刚装上只有一条抄表记录**也有图：用电量要两条记录相减才算得出来，但剩余电量是实打实的，所以那一天 / 那一月照样画出来，剩余电量画成一个标了数值的点，不是整块空白。
  ![只有一条记录](docs/main-single-reading.png)
- **自动查询**：固定**每到整点和半点（`:00:00` / `:30:00`）**各读一次，读到的数按整点记账（3:00 的读数算 2 点那一格）。半点那一次落在**同一格**上：查到的值变了就顶替掉这一格里原来那条，一样就当没这回事，不写盘也不重画——学校两三个小时才抄一次表，多这一次不会凭空多出数据点，但抄表一上传就能早半小时看到。程序刚启动时也会立刻查一次，这一次落在哪个小时就记在**那个**小时上（23:42 启动查到的记在 23 点，不会借用还没到的 0 点那一格；只有差不到 5 分钟就到下一个整点时才算下一格，为的是兜住轮询早几十毫秒的情况）。没有间隔可调，也没有"手动刷新"按钮——想到的时候点一下反而只会拿到跟上次一样的 `updatedt`。查失败就 1 分钟后重试。
- **充值**：主界面和托盘菜单都能开，充的是**主界面上正看着的那一间**（切了房间，正开着的充值窗口跟着换，手里那一单顺带作废）。选金额（预设 7 档，或者直接在自定义那格里打数字，非空且在 1~1000 元之间就能直接下单，回车也算）→ 点"生成付款码"**弹一个码窗**，二维码底下就是操作按钮 → 手机扫码付。码是限时的（学校那边给几分钟），**过期自动重新下一单换一张新的**，跟网页行为一致，也可以自己点"换一张"；**取消这一单**（或者直接关掉码窗）就作废，重新申请拿到的一定是新的一张。每 3 秒查一次订单，付上了立刻刷一次电量和记录，低电量提醒也跟着解除。**不用填手机号**。
...
- **设置**：分成**宿舍 / 提醒 / 组件 / 数据**四页（顶上一条分段选择器切页，每组一张玻璃卡，组名写在卡外面），所有改动先记在配置副本上，点**保存**才生效、才落盘，点**取消**或直接关窗口什么都不动。数字框也是自己画的（系统 `NumericUpDown` 在深色主题下那两个小箭头永远是浅色的）：中间能直接打字，两头 `−` / `+` 按住连发，滚轮和上下键也能调。
...
- **低电量还能发一封邮件**：走 QQ 邮箱的 SMTP（`smtp.qq.com:587` + STARTTLS），跟系统通知同一个触发条件、同一份"充过电才再提醒"的状态，通知中心没看见也不会漏。触发条件有两个，满足哪个都提醒：**剩余电量低于阈值**，或者**照眼下的日均算预计可用不足设置里那个天数**（后者默认 0，也就是不看这一条；一开空调日均能翻几倍，等跌到阈值可能已经半夜断电了，所以想稳一点就调到 0.5 天上下）。两条线是**几间共用**的，在"提醒"页顶上调。
  **发件箱共用、收件人按间配**：一个 QQ 授权码只对应一个邮箱，所以发件地址和授权码填一份（"提醒"页中间那张卡）；**弹不弹通知、发不发邮件、发到哪几个邮箱，是每间各自一套**（都默认关、默认空），在下面那张卡里先选房间再配。旁边有个**试一封邮件**，不用先保存就能当场发一封验证，信里带的是**这一间**眼下的数字。
...
- `config.json` —— 在记录哪几间（`Dorms`，每间带自己的通知 / 邮件开关和收件人）、正看着哪一间、两条提醒线（`LowThreshold` 度数阈值 / `LowDaysThreshold` 预计可用天数）、共用的发件箱和授权码（`MailFrom` / `MailAuthCode`，**明文**）、组件开关和位置、粒度
- `readings-B43-R422.jsonl` —— **一间一个文件**，文件名里就是房号。一行一条整点读数，按 `SlotTime` 认格子（半点那一次也落在本整点这一格）。**读到的跟这一格已有的一模一样就不写新行**（电表两三个小时才上传一次，同一格里反复查到的值往往完全相同），值变了才追加一行、载入时后写的赢；**启动载入完还会把文件收拢成一格一行**，多出来的重复行和坏行一并清掉（先写临时文件再整体替换）。所以文件行数和图表上的格子数是对得上的。载入时整点一律按 `FetchedAt` 重算，老版本盖错的章会自动纠回来
...
src/BillSystem/
  ...
  Models/
    Dorm.cs             一间宿舍：房号 + 这一间自己的通知 / 邮件开关和收件人
    AppConfig.cs        配置（宿舍名单、共用的阈值和发件箱、组件、粒度）+ 读写
    Reading.cs / RechargeRecord.cs / Enums.cs
  Services/
  ...
  UI/
    ...
    SettingsForm.cs     设置四页：宿舍 / 提醒 / 组件 / 数据（改副本，保存才生效）
